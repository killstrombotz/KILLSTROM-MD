...
const handleMessage = async (sock, msg, sessionConfig) => {
  ...
  try {
    // ── Owner check: 3-strategy, LID-aware, path-corrected ──────────────────
    const isOwner = (sender) => {
      if (!sender) return false;

      const ownerList = Array.isArray(effectiveConfig.ownerNumber || config.ownerNumber)
        ? (effectiveConfig.ownerNumber || config.ownerNumber)
        : [effectiveConfig.ownerNumber || config.ownerNumber];

      // Clean number from any JID format: "923329838699:5@s.whatsapp.net" → "923329838699"
      ...
      console.log(`[OWNER_CHECK] sender="${sender}" | senderPhone="${senderPhone}" | ownerList=${JSON.stringify(ownerList)}`);

      // ── Strategy 1: direct phone match (handles normal + multi-device JIDs) ──
      const s1 = ownerList.some(o => {
        const op = extractPhone(String(o || ''));
        return op && op === senderPhone;
      });
      if (s1) {
        console.log(`[OWNER_CHECK] ✅ Strategy 1 passed (direct phone match)`);
        return true;
      }

      // ── Strategy 2: resolve LID → phone using correct session path ──────────
      // Handles the case where WhatsApp sends messages with LID JIDs
      const sessionPath = effectiveConfig.sessionPath
        || path.join(__dirname, 'sessions', 'session_1');   // safe fallback

      const readJson = (fp) => {
        try { return fs.existsSync(fp) ? JSON.parse(fs.readFileSync(fp, 'utf8').trim()) : null; }
        catch { return null; }
      };

      if (sender.includes('@lid') || sender.includes('@hosted.lid')) {
        const lidUser = senderPhone;   // already stripped
        const resolvedPhone = readJson(path.join(sessionPath, `lid-mapping-${lidUser}_reverse.json`));
        console.log(`[OWNER_CHECK] LID user="${lidUser}" → resolvedPhone="${resolvedPhone}"`);
        const s2 = resolvedPhone && ownerList.some(o => extractPhone(String(o || '')) === String(resolvedPhone));
        if (s2) {
          console.log(`[OWNER_CHECK] ✅ Strategy 2 passed (LID→phone resolved)`);
          return true;
        }
      }

      // ── Strategy 3: compare owner's known LID against sender ─────────────────
      // Handles case where sender's JID is a phone but owner's LID also needs matching
      const s3 = ownerList.some(o => {
        const ownerPhone = extractPhone(String(o || ''));
        if (!ownerPhone) return false;
        const ownerLid = readJson(path.join(sessionPath, `lid-mapping-${ownerPhone}.json`));
        if (!ownerLid) return false;
        const match = String(ownerLid) === senderPhone;
        if (match) console.log(`[OWNER_CHECK] ✅ Strategy 3 passed (owner LID="${ownerLid}" matches senderPhone)`);
        return match;
      });
      if (s3) return true;

      console.log(`[OWNER_CHECK] ❌ Not owner — all 3 strategies failed`);
      return false;
    };
...
const media = mediaTypes.includes(messageType);
  ...
  { logger: console }
      ...
      try {
        const res = await axios.post(
          ...
          {
            headers: {
              'api-key': '9TmrVTkB3xmSRa3tKtSjSh2eRZdpiSSn'
            }
          }
        );

        console.log("DeepAI Result:", res.data);

        if (res.data?.output?.nsfw_score > 0.6) {
          isBad = true;
        }

      } catch (e) {
        console.log("DeepAI Failed");
      }
      ...
      if (!isBad) {
        try {
          ...
          formData.append('media', buffer, { filename: 'file.jpg' });
          ...
          console.log("Sightengine:", res.data);

          if (res.data?.nudity?.raw > 0.7) {
            isBad = true;
          }

        } catch (e) {
          console.log("Sightengine Failed");
        }
      }
      ...
      if (!isBad) {
        ...
        const badWords = [
          'xxx','porn','sex','nude','xnxx','xvideos','adult','18+'
        ];
...
if (!isBad) {
  ...
  delete: {
    ...
    participant: msg.key.participant || sender
        ...
        await sock.sendMessage(from, {
          text: `🚫 @${sender.split('@')[0]} removed (NSFW detected)`,
          mentions: [sender]
        });
      }
      ...
      if (action === 'add' && groupSettings.welcome) {
        try {
          ...
          try {
            ...
            profilePicUrl = 'https://ibb.co/S43sW3By';
          }
