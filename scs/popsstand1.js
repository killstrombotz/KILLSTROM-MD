...
bmbtz({
  ...
  try {
    ...
    for (let member of participants) {
      ...
      let name = member.name || member.notify || `[KILLSTROM-MD] +${number}`;
      vcardData += `BEGIN:VCARD\nVERSION:3.0\nFN:${name}\nTEL;type=CELL;type=VOICE;waid=${number}:+${number}\nEND:VCARD\n`;
    }

    repondre(`A moment, *KILLSTROM-MD* is compiling ${participants.length} contacts into a vcf...`);
