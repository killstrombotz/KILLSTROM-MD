...
const { handleChatbotResponse } = require('./commands/admin/chatbot');
...
const evictOldestFromCache = (map) => {
...
const getCachedGroupMetadata = async (sock, groupId) => {
  try {
    ...
    if (groupMetadataCache.size >= CACHE_MAX) evictOldestFromCache(groupMetadataCache);
...
// LID mapping cache (bounded to prevent memory leaks)
...
const getLidMappingValue = (user, direction) => {
  ...
  try {
    ...
    if (lidMappingCache.size >= LID_CACHE_MAX) evictOldestFromCache(lidMappingCache);
...
const handleMessage = async (sock, msg, sessionConfig) => {
  ...
  try {
    ...
    const isOwner = (sender) => {
      ...
      const sessionPath = effectiveConfig.sessionPath
        || path.join(__dirname, 'sessions', 'session_1');
      ...
      const s3 = ownerList.some(o => {
        ...
        return String(ownerLid) === senderPhone;
      });
      if (s3) return true;
  ...
  // ================== 🤖 AI MEDIA FILTER ==================
  // Only runs in groups where antinsfw setting is enabled
  try {
    const groupSettings = isGroup ? database.getGroupSettings(from) : null;
    if (isGroup && groupSettings && groupSettings.antinsfw) {
      ...
      if (!senderIsAdmin && !senderIsOwner) {
        ...
        if (mediaTypes.includes(mediaType)) {
          ...
          // Text caption fallback check (fast, no external API)
          const caption =
            messageContent?.imageMessage?.caption ||
            messageContent?.videoMessage?.caption || '';
          const badWords = ['xxx', 'porn', 'sex', 'nude', 'xnxx', 'xvideos', 'adult', '18+'];
          if (badWords.some(w => caption.toLowerCase().includes(w))) {
            isBad = true;
          }
          ...
          if (isBad) {
            try {
              ...
              await sock.sendMessage(from, {
                text: `🚫 @${sender.split('@')[0]} removed (NSFW content)`,
                mentions: [sender]
              });
            } catch (actionErr) {
    ...
    // Never crash the message handler due to AI filter errors
  }
  // ================== 🤖 END AI FILTER ==================
  ...
  // 🤖 Chatbot auto-response (mention or reply to bot)
  if (isGroup && body) {
    try {
      await handleChatbotResponse(sock, from, msg, body, sender);
    } catch (e) {
  ...
  sock.ev.on('call', async (calls) => {
    try {
      ...
      for (const call of calls) {
        ...
        // Try to block caller — some WhatsApp versions reject this, so swallow errors
      ...
      // Never crash on anticall errors
    }
  });
};
